...
def parse_args():
    ...
    parser.add_argument("--expert-data", default="expert_data/BlockedHalfCheetah_backward",type=str,
    ...
    parser.add_argument("--l1-ratio", type=float, default=0.9,
        help="the discount factor gamma")
...
def train_step(agent_state,anti_buffer_state):
    ...
    def update_value_critic(
        ...
        def mse_loss(params):
            ...
            discounted_rewards = (
               (1 - Batch.dones) * args.gamma  * next_V
            )
            loss = (
    ...
    def update_critic(
        ...
        def mse_loss(params):
            ...
            coeff=args.l1_ratio
            loss =  coeff*(- w.mean())+vf_l2_loss+(1-coeff)*(((current_Q-next_V)**2).mean())+args.reg_cof*((anti_Q-anti_Q_target)**2).mean()+((current_expert_Q-current_expert_Q_target)**2).mean()
...
def eval_policy(envs, agentstate,global_step,round=10):
    ...
    if args.track:
        ...
        infos={key: np.stack([d[key] for d in traj_info]) for key in traj_info[0].keys() if isinstance(traj_info[0][key], np.ndarray)}
        x = np.arange(len(infos['x_position']))
        y = infos['x_position']
        plt.scatter(x, y)
        wandb.log({"xy_plot": wandb.Image(plt)})
        plt.close()
        x = np.arange(len(infos['x_velocity']))
        y = infos['x_velocity']
        plt.scatter(x, y)
        wandb.log({"xy_v_plot": wandb.Image(plt)})
        plt.close()
        # x=infos['x_velocity'].sum()
        # y=infos['y_velocity'].sum()
        # x_norm = x/np.sqrt(x**2+y**2)
        # y_norm = y/np.sqrt(x**2+y**2)

        # theta = np.arctan2(y_norm, x_norm)
        # wandb.log({"global_step": global_step,"theta": theta})
...
if __name__ == "__main__":
    ...
    # env_module.theta=args.theta*jnp.pi
    # Create networks
    ...
    actor = Actor(action_dim=np.prod(envs.action_space.shape),log_std_max=-5)
    ...
    state={"qf_state":qf_state,"actor_state":actor_state, "vf_state": vf_state}
    state=checkpointer.restore("/home/guorui/jax-rl/sacpolicy/BlockedHalfCheetah/BlockedHalfCheetah__sm_test_mask_new_data__1__1695468677664/", item=state)
    qf_state=state['qf_state']
    ...
    total_step=int((args.total_timesteps - args.learning_starts) / 50)
    for global_step in range(total_step):
        ...
        if global_step%20 == 0:
            log_data = {
                ...
                # "charts/expert_log_prob":get_log_prob(agentstate.actor_state.params,expert_action,expert_obs),
            }
            log_data = (
                log_data
