...
def train_step(agent_state,anti_buffer_state):
    ...
    def update_reward_value_critic(
        ...
        def mse_loss(params):
            ...
            y = current_reward_Q - current_reward_V + (current_Q - current_V).clip(args.cost_min, args.cost_max)
    ...
    def update_critic(
        ...
        decay_weight = (
            100*(1- 10*(vf_state.opt_state[0].count / args.total_timesteps))
        ).clip(1)
        ...
        def mse_loss(params):
            ...
            coeff=args.l1_ratio
            loss =  coeff*(- w.mean())+vf_l2_loss+(1-coeff)*(((current_Q-next_V)**2).mean())+decay_weight*args.reg_cof*((anti_Q-anti_Q_target)**2).mean()+((current_expert_Q-current_expert_Q_target)**2).mean()
...
def eval_policy(envs, agentstate,round=10):
    ...
    while ~mask.all():
        ...
        mask|=(dones | truncated)

        obs = next_obs
        cum_reward[~mask] += rewards[~mask]
        cum_cost[~mask] += costs[~mask]
        lengths[~mask] += 1
    log_data = {
        "charts/mean_return": np.average(cum_reward),
        "charts/mean_cost": np.average(cum_cost),
        "charts/mean_length": np.average(lengths),
    }
...
if __name__ == "__main__":
    ...
    num_envs=20
    envs, cost_function,env_module = make_cost_env_no_xla(args.env_id, num_envs, args.seed,get_module=True)
    ...
    for global_step in range(int((args.total_timesteps - args.learning_starts) / 50)):
        if global_step%500==0:
            ...
            # checkpointer.save(f"debugpolicy/{args.env_id}/{run_name}/{global_step}", state, save_args=save_args)
