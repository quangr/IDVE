...
def parse_args():
    ...
    parser.add_argument("--anti-cof", type=float, default=2,
        help="the reward alpha parameter")
    ...
    parser.add_argument('--sac-actor', type=lambda x:bool(strtobool(x)), default=True, nargs="?", const=False,)
...
def reward_train_step(agent_state):
    ...
    def update_critic(
        ...
        next_values = vf.apply(vf_state.params, Batch.next_observations).reshape(
            -1
        )
        ...
        def mse_loss(params):
            ...
            discounted_rewards =  (1 - Batch.dones) * args.gamma * (
                next_values
            )
            loss = ((current_Q - discounted_rewards) ** 2).mean()
            return loss, current_Q.mean()

        (update_qf_loss_value, update_qf_values), grads = jax.value_and_grad(
            mse_loss, has_aux=True
        )(update_qf_state.params)
        update_qf_state = update_qf_state.apply_gradients(grads=grads)
        callback_log["update_qf_loss_value"] = update_qf_loss_value
        callback_log["update_qf_values"] = update_qf_values
        return update_qf_state
    ...
    def update_reward_value_critic(            
        ...
        next_V = vf.apply(vf_state.params, Batch.observations * single_mask).reshape(-1)
    ...
    def update_actor(
        ...
        if args.sac_actor:
            ...
            def actor_loss(params):
                ...
                qf_pi = qf.apply(
                    reward_qf_state.params, Batch.observations, actions
                ).reshape(-1) + (
                    qf.apply(qf_state.params, Batch.observations, actions).reshape(-1)
                    - vf_pi
                ).clip(
                    args.cost_min, args.cost_max
                )
                actor_loss = ((0.001 * log_prob - qf_pi)).mean()
    ...
    update_qf_state = soft_update(args.tau, update_qf_state)
...
def train_step(agent_state, anti_buffer_state):
    ...
    def update_value_critic(
        ...
        def mse_loss(params):
            ...
            # loss = (
            #     (current_Q.reshape(-1) - discounted_rewards) ** 2
            # ).mean()
            loss = (
                (discounted_rewards - current_Q.reshape(-1)).clip(0)
            ).mean() - next_V.clip(max=0).mean() * 0.1
            return loss, current_Q.mean()
    ...
    def update_critic(
        ...
        def mse_loss(params):
            ...
            loss = (
                coeff * (-w.mean())
                + (1 - coeff) * (((current_expert_Q - 0) ** 2).mean())
                + args.reg_cof * (((anti_Q - 0) ** 2).mean())
...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time()*100)}"
    if args.track:
        import wandb
    ...
    # random_indice = np.random.randint(0, len(_expert_data["traj_obs"]), size=2000)
    ...
    update_qf_state = RLTrainState.create(
        apply_fn=qf.apply,
        params=qf.init({"params": qf_key}, obs, action),
        target_params=qf.init({"params": qf_key}, obs, action),
        tx=optax.adam(learning_rate=args.q_lr),
    ...
    def update_anti_actions(anti_buffer_state, actor_state):
        ...
        index = jnp.argpartition(log_prob, 100)[:100]
    ...
    total_step = int((args.total_timesteps - args.learning_starts) / 500)
    reward_gap = float("inf")
    for global_step in range(total_step):
        if global_step % 50 == 0:
            eval_policy(envs, agentstate, global_step)
        ...
        reward_gap = jnp.abs(
            reward_callback_log["reward_vf_loss_value"].mean()
            - old_reward_vf_loss_value
        )
        old_reward_vf_loss_value = reward_callback_log["reward_vf_loss_value"].mean()
        if global_step % 2 == 0:
            log_data = log_data | {
    ...
    if args.track:
        ...
        plt.close()
    eval_policy(envs, agentstate, total_step, 100)
    state = {
