...
def parse_args():
    ...
    parser.add_argument("--env-id", type=str, default="BiasedSwimmer_ls",
        help="the id of the environment to be used")
    parser.add_argument("--expert-data", type=str, default="expert_data/BiasedSwimmer_ls",
...
def train_step(agent_state):
    ...
    def update_reward_value_critic(
        ...
        def mse_loss(params):
            ...
            current_next_V = vf.apply(params, Batch.next_observations).reshape(-1)
    ...
    def update_actor(
        ...
        current_reward_V = vf.apply(vf_state.params, Batch.observations).reshape(
            -1
        )
        current_reward_Q = qf.apply(
            qf_state.target_params, Batch.observations, Batch.actions
