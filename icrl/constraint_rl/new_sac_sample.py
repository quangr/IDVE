...
def split_data(expert_data):
    ...
    expert_obs = expert_data["traj_obs"]
    expert_next_obs = expert_data["traj_next_obs"]
    expert_dones = expert_data["dones"]
    expert_rewards = expert_data["rewards"]
...
if __name__ == "__main__":
    ...
    expert_actor_state=checkpointer.restore("/home/guorui/jax-rl/bcpolicy/BlockedHalfCheetah_backward/BlockedHalfCheetah_backward__behavior_clone__0__16954460932/", item=actor_state)
    ...
    if args.autotune:
        ...
        ent_coef_state = TrainState.create(
            ...
            tx=optax.adam(learning_rate=args.q_lr*0.05),
        )
    ...
    if args.expert_data is not None:
        ...
        for i in range(len(expert_obs)):
            rb.add(expert_obs[i], expert_next_obs[i], expert_action[i], expert_rewards[i], expert_dones[i],None)
    ...
    rollout_step=np.random.randint(0, 1000)
    step=0
    for global_step in generator:
        ...
        if dones.any() or truncated.any():
            ...
            print(f"global_step={global_step + 1}, episodic_return={cum_reward}, episodic_cost={cum_cost}")
    ...
    if args.expert_data is None:
        data = (
            ...
            rb.dones,
            rb.rewards,
            {key: np.stack([d[key] for d in traj_info]) for key in traj_info[0].keys() if isinstance(traj_info[0][key], np.ndarray)}
        )
    else:
        data = (
            rb.observations[len(expert_obs):],
            rb.next_observations[len(expert_obs):],
            rb.actions[len(expert_obs):],
            rb.dones[len(expert_obs):],
            rb.rewards[len(expert_obs):],
            {key: np.stack([d[key] for d in traj_info]) for key in traj_info[0].keys() if isinstance(traj_info[0][key], np.ndarray)}
        )
    save_args = orbax_utils.save_args_from_target(data)
    checkpointer.save(f"tmp/buffer/{args.env_id}/{run_name}", data, save_args=save_args)
