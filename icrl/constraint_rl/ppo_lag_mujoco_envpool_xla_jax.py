...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="constraint_rl",
...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}__{int(100*time.time())}"
    if args.track:
        import wandb
    ...
    def get_wrappers(envs):
        ...
        return wrappers
    # wrappers=[]
    envs, handle, step_env, episode_stats,cost_function =make_cost_env(args.env_id,args.num_envs,args.seed,get_wrappers)
    ...
    def ppo_loss(params, x, a, logp, reward, cost, truncated, penalty):
        ...
        loss = pg_loss - args.ent_coef * entropy_loss + (v_loss + cost_v_loss) * args.vf_coef
        return loss, (
            pg_loss,
            v_loss,
            cost_v_loss,
