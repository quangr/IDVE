...
from icrl.env.envs.grid import WallGridworld


def set_transition_matrix(env):
    transition_matrix=np.zeros((7,7,8,2))
    action_mask=np.zeros((7,7,8))
    fix_rewards=np.ones((7,7,8))
    for x in range(7):
        for y in range(7):
            if [x,y] in env.terminals:
                for action in range(8):
                    transition_matrix[x,y,action]=np.array([x,y])
                    fix_rewards[x,y,action]=0
                    action_mask[x,y,action]=1
            else:
                for action in range(8):
                    env.reset_with_values(info_dict={'states': [x, y]})
                    s_primes, rewards, dones, ts, infos = env.step(action)
                    transition_matrix[x,y,action]=np.array([int(s_primes[0]),int(s_primes[1])])
                    fix_rewards[x,y,action]=rewards
                    action_mask[int(s_primes[0]),int(s_primes[1]),infos['admissible_actions']]=1
    return transition_matrix.astype(np.int32),fix_rewards,action_mask

def update_policy(transition_matrix,action_mask,fix_rewards):
    vs=np.zeros((7,7))
    policy=np.ones((7,7,8))/8
    updated_policy=np.zeros((7,7))
    for aolianfei in range(500):
        for n_update in range(100):
            _vs=vs.copy()
            for x in range(7):
                for y in range(7):
                    p=policy[x,y]
                    newv=[_vs[transition_matrix[x,y,action,0],transition_matrix[x,y,action,1]] for action in range(8)]
                    vs[x,y]=(p*(fix_rewards[x,y]+0.9*np.array(newv))).sum()
            if (_vs-vs<0.01).all():
                break
        policy_stable=True
        for x in range(7):
            for y in range(7):
                newv=[vs[transition_matrix[x,y,action,0],transition_matrix[x,y,action,1]] for action in range(8)]
                p=np.zeros(8)
                v=fix_rewards[x,y]+0.9*np.array(newv)
                for a in range(8):
                    if action_mask[x,y,a]==0:
                        v[a]=-np.inf
                maxlist=np.argwhere(v == np.amax(v))
                p[maxlist]=1/len(maxlist)
                if policy_stable:
                    policy_stable=(policy[x,y]==p).all()
                policy[x,y]=p
                updated_policy[x,y]=v.argmax()
        # print(vs)
        # print_traj(updated_policy)
        if policy_stable:
            return policy

def generate_trajectory(policy, transition_matrix,start,end):
    def dfs(state,traj):
        traj_set=[]
        if state!=end:
            p=policy[state[0],state[1]]
            maxlist=np.argwhere(p>0)
            for action in maxlist:
                new_state=tuple(transition_matrix[state[0],state[1],int(action)].tolist())
                traj_copy = traj.copy()
                traj_copy.append(list(new_state))
                traj_set = traj_set + dfs(new_state, traj_copy)
        else:
            traj_set.append(np.array(traj))
        return traj_set
    return dfs(start,[start])    

def get_policy_trajectory(env):
    transition_matrix,fix_rewards,action_mask=set_transition_matrix(env)
    policy=update_policy(transition_matrix,action_mask,fix_rewards)
    start=(env.start_states[0][0],env.start_states[0][1])
    end=(env.terminals[0][0],env.terminals[0][1])
    traj = generate_trajectory(policy, transition_matrix,start,end)
    return traj
...
with open(f'expert_data/grid_world/{settingname}.yaml') as f:
    ...
    reward_states = config['reward_states']
    terminal_states = config['terminal_states']
    unsafe_states = config['unsafe_states']
    start_states = config['start_states']

    env=WallGridworld(map_height=map_height, map_width=map_width, n_actions=n_actions, transition_prob=transition_prob, reward_states=reward_states, terminal_states=terminal_states, unsafe_states=unsafe_states, start_states=start_states)
    expert_trajs = get_policy_trajectory(env)
    second_env=WallGridworld(map_height=map_height, map_width=map_width, n_actions=n_actions, transition_prob=transition_prob, reward_states=[[0,6]], terminal_states=[[0,6]], unsafe_states=unsafe_states, start_states=[[6,0]])
    second_expert_trajs = get_policy_trajectory(second_env)
...
expert_indice = np.random.permutation(len(obs_E))[:20]
obs_E=obs_E[expert_indice]
obs_next_E=obs_next_E[expert_indice]

np.add.at(rho_E, (obs_E[:,0],obs_E[:,1],obs_next_E[:,0],obs_next_E[:,1]), 1)
np.add.at(rho_S, (obs_S[:,0],obs_S[:,1],obs_next_S[:,0],obs_next_S[:,1]), 1)
...
np.add.at(state_ocuupy_E, (obs_E[:,0],obs_E[:,1]), 1)
