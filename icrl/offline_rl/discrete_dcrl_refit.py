...
gird_shape = (7, 7)
expert_trajs = np.load("expert_data/grid_world/setting2/expert_trajs.npz")
...
random_path = [
    generate_random_path(np.ones(gird_shape), (1,4), end_point)
    for i in range(10)
]
sample_trajs += random_path
if (0,6) !=end_point:
    random_path = [
        generate_random_path(np.ones(gird_shape), (0, 6), end_point) for i in range(20)
    ]
    sample_trajs += random_path
if (6,0) !=end_point:
    random_path = [
        generate_random_path(np.ones(gird_shape), (6, 0), end_point) for i in range(20)
    ]
    sample_trajs += random_path
if (0,0) !=end_point:
    random_path = [
        generate_random_path(np.ones(gird_shape), (0, 0), end_point) for i in range(20)
    ]
    sample_trajs += random_path
if (6,6) !=end_point:
    random_path = [
        generate_random_path(np.ones(gird_shape), (6, 6), end_point) for i in range(20)
    ]
    sample_trajs += random_path
...
# @jax.jit
# def refitV(reward_V, V,old_reward_V):

#     (y_E, V_E, next_V_E), (y_S, V_S, next_V_S) = get_V(reward_V)
#     (old_y_E, old_V_E, old_next_V_E), (old_y_S, old_V_S, old_next_V_S) = get_V(old_reward_V)
#     (cost_y_E, cost_V_E, next_cost_V_E), (cost_y_S, cost_V_S, next_cost_V_S) = get_V(V)
#     cost=100000000* (
#         (1 - beta) * (d_S * (jnp.abs(cost_V_S) + cost_V_S.clip(0))).mean()
#         + (beta) * (d_E * (jnp.abs(cost_V_E)+ cost_V_E.clip(0))).mean()
#     )
#     cost_y_E = cost_y_E.clip(cost_min, cost_max)
#     cost_y_S = cost_y_S.clip(cost_min, cost_max)
#     return (
...
#         + lamb
#         * (
#             beta * (fp_star(old_y_E + r_E + cost_y_E) * d_E).mean()
#             + (1 - beta) * (fp_star(y_S + r_S + cost_y_S) * d_S).mean()
#         )
#         + 100000*((old_V_E-V_E)**2).mean()
#         + 100000*((next_V_E-old_next_V_E)**2).mean()
#         +cost
#     )
...
def fit(V, optimizer):
    ...
    @jax.jit
    def refit_step(V, reward_V,old_reward_V, opt_state):
        loss_value, grads = jax.value_and_grad(refitV,argnums=(0,1))(reward_V, V,old_reward_V)
        updates, opt_state = optimizer.update(grads, opt_state, (V,reward_V))
        (V,reward_V) = optax.apply_updates((V,reward_V), updates)
        return V, reward_V, opt_state, loss_value
    ...
    # for epoch in range(10):
    #     for i in range(1000):
    #         V, reward_V, refit_opt_state, loss_value=refit_step(V, reward_V,old_reward_V, refit_opt_state)
    #     saveimg(reward_V,f"grid_img/test/refit/reward_V/{epoch}.jpg")
    #     saveimg(V,f"grid_img/test/refit/V/{epoch}.jpg")
