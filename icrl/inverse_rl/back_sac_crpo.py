...
@jax.jit
@checkify.checkify
def train_step(buffer_state, qf_state,reward_qf_state, actor_state,key):
    ...
    expert_indice = jax.random.randint(sample_key,(args.batch_size,),0, len(expert_obs), )
    ...
    return actor_state,qf_state,reward_qf_state,qf_loss_value,qf_values,reward_qf_loss_value, reward_qf_values,observations,key
if __name__ == "__main__":
    ...
    def updata_eval_critic(
        ...
        next_q_values = next_q_values - 0.01 * next_log_prob.reshape(-1, 1)
    ...
    # w_init = jnp.array([1.,0.1])
    # lbfgsb = ScipyBoundedMinimize(fun=_estep_dual_loss, method="l-bfgs-b")
    ...
    # pg = ProjectedGradient(fun=_estep_dual_loss, projection=projection_box)
    @jax.jit
    def update_actor(
        actor_state: RLTrainState,
        qf_state: RLTrainState,
    ...
    def E_step(actor_state, qf_state,dual, reward_qf_state, observations, key):
        ...
        update_q=jax.nn.softmax(((cost_q-lamb*reward_q)/eta).squeeze(),axis=1)
        return old_mean,old_log_std,actions,update_q,dual
    ...
    for global_step in generator:
        ...
        if global_step > args.learning_starts:
            ...
            _,(actor_state,qf_state,reward_qf_state,qf_loss_value,qf_values,reward_qf_loss_value, reward_qf_values,observations,key) = train_step(buffer_state, qf_state,reward_qf_state, actor_state, key)
            if(np.average(reward_deque)>-5.5):
                ...
                (actor_state, reward_qf_state, actor_loss_value, key, entropy) = update_actor(
                    actor_state,
                    reward_qf_state,
                    ent_coef_value,
                    observations,
            ...
            if global_step % 100 == 0:
                ...
                if args.track:
                    wandb.log({"global_step": global_step,"losses/qf_values": qf_values.mean().item(), 
                            "losses/qf_loss": qf_loss_value.item(), "losses/actor_loss": actor_loss_value.item(), 
                            "losses/reward_qf_loss_value": reward_qf_loss_value.item(), "reward_losses/qf_values": reward_qf_values.mean().item(), 
                            "losses/alpha": ent_coef_value.item(),"charts/mean_return": np.average(reward_deque),
