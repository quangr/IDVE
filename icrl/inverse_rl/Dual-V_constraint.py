...
class Lagrange(flax.struct.PyTreeNode):
    ...
    def update_lagrange_multiplier(self, Jc: float):
        ...
        """
        compute_lambda_loss=partial(self.compute_lambda_loss,cost_limit=self.cost_limit)
...
@jax.jit
@checkify.checkify
def train_step(buffer_state, vf_state,qf_state, actor_state,reward_lag, key):
    ...
    actor_state,vf_state,reward_lag, (vf_loss_value, vf_values, qf_values,estimate_adv,actor_loss_value,entropy), key = update_value_critic_and_actor(
        actor_state,
        qf_state,
        vf_state,
        Policy_Batch,
    ...
    return actor_state,vf_state,qf_state,qf_loss_value,qf_values,vf_loss_value, vf_values,actor_loss_value,estimate_adv,reward_lag,key
...
if __name__ == "__main__":
    ...
    def update_critic(
        ...
        policy_next_values =vf.apply(vf_state.params,Policy_Batch.next_observations).clip(Q_min,Q_max)
        # expert_next_values =vf.apply(vf_state.params,Expert_Batch.next_observations).clip(Q_min,Q_max)
        def mse_loss(params):
            ...
            policy_discounted_rewards=((-1/reg_mult)+(1 - Policy_Batch.dones).reshape(-1, 1)*args.gamma*policy_next_values).squeeze(-1)
            # expert_discounted_rewards=((1/reg_mult)+(1 - Expert_Batch.dones).reshape(-1, 1)*args.gamma*expert_next_values).squeeze(-1)
            loss = ((expert_current_Q.reshape(-1) - Q_max)**2).mean()+((policy_current_Q.reshape(-1) - policy_discounted_rewards)**2).mean()
    ...
    def update_value_critic_and_actor(
        actor_state: RLTrainState,
        qf_state: RLTrainState,
        vf_state: RLTrainState,
        Policy_Batch: BatchData,
        ...
        def lag_loss(params):
            ...
            current_V = vf.apply(
                vf_state.params, Batch.observations
            ).reshape(-1)
            rewards=Batch.rewards
            rewards=rewards-rewards.mean()
            y=(current_Q-current_V)
            estimate_adv=(((y/2+1).clip(0)-1)*rewards).mean()
            new_y=y+params*rewards
            loss=((((new_y/2+1).clip(0)-1)*rewards).mean()**2)*(estimate_adv<0)
            return loss,estimate_adv

        (lag_loss_value,estimate_adv), grads = jax.value_and_grad(lag_loss, has_aux=True)(
            reward_lag.state.params
        )
        new_reward_lag=reward_lag.state.apply_gradients(grads=grads)
        reward_lag=reward_lag.replace(state=new_reward_lag.replace(params=jnp.clip(new_reward_lag.params, 0)))


        alpha=reward_lag.state.params*(estimate_adv<0)
        ...
        def mse_loss(params):
            ...
            rewards=Batch.rewards
            rewards=rewards-rewards.mean()
            w=((y+alpha*rewards)/2+1).clip(0)
            loss=(1-lamb)*(current_V).mean()+lamb*(w*y-(w-1)**2).mean()
        ...
        def actor_loss(params):
            ...
            vf_pi = vf.apply(vf_state.params, Batch.observations).reshape(-1)
            ...
            rewards=Batch.rewards
            rewards=rewards-rewards.mean()
            y=min_qf_pi-vf_pi+alpha*rewards
            
            actor_loss = -(jnp.exp(((y/2+1).clip(0))*10.).clip(max=100.).flatten() * log_prob ).mean()
        ...
        return (
            actor_state,vf_state,reward_lag,
            (vf_loss_value, vf_values, qf_values,estimate_adv,actor_loss_value,entropy),
            key,
        )
    ...
    def update_actor(
        ...
        def actor_loss(params):
            ...
            actor_loss = -(jnp.exp((min_qf_pi-vf_pi)*10.).clip(max=100.).flatten() * log_prob ).mean()
            return actor_loss, -log_prob.mean()
    ...
    def train_step_body(carry,step):
        buffer_state, actor_state,vf_state,qf_state,reward_lag, key = carry
        ...
        _, (actor_state,vf_state,qf_state,qf_loss_value,qf_values,vf_loss_value, vf_values,actor_loss_value,estimate_adv,reward_lag,key) = train_step(buffer_state, vf_state,qf_state, actor_state,reward_lag, key)
        return (buffer_state, actor_state,vf_state,qf_state,reward_lag,key), (qf_loss_value,qf_values.mean(),vf_loss_value, vf_values.mean(),actor_loss_value,reward_lag.state.params,estimate_adv)
    for global_step in range(int((args.total_timesteps-args.learning_starts)/50)):
        ...
        (buffer_state, actor_state,vf_state,qf_state,reward_lag, key), (qf_loss_value,qf_values,vf_loss_value, vf_values,actor_loss_value,lag,estimate_adv) = jax.lax.scan(
            train_step_body,
            (buffer_state, actor_state,vf_state,qf_state,reward_lag, key),
            (),
            length=50,
        )
        ...
        log_data = {
            ...
            "losses/estimate_adv": estimate_adv.mean().item(),
            "losses/lag": reward_lag.state.params,
