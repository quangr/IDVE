...
def train_step(agent_state):
    ...
    def update_reward_critic(
        ...
        # ).reshape(-1)
        # current_V = vf.apply(vf_state.params, Batch.observations).reshape(-1)

        next_values = vf.apply(reward_vf_state.params, Batch.next_observations).reshape(
            -1
        )

        next_cost_values = vf.apply(vf_state.params, Batch.next_observations).reshape(
            -1
        )
        ...
        def mse_loss(params):
            ...
            discounted_rewards = (
                ...
                + (1 - Batch.dones) * args.gamma * (next_values+next_cost_values)
            )
    ...
    def update_reward_value_critic(
        ...
        def mse_loss(params):
            ...
            y=current_reward_Q - current_reward_V - current_V
            
            loss = (1 - lamb) * (current_reward_V).mean() + lamb * ((y+y**2/2).clip(0)).mean()
    ...
    def update_value_critic_and_actor(
        ...
        decay_weight=(vf_state.opt_state[0].count%(args.total_timesteps/args.decay_cycle))/(args.total_timesteps/args.decay_cycle)
        callback_log["decay_weight"]=decay_weight

        def mse_loss(params):
            current_V = vf.apply(params, Batch.observations).reshape(-1)
            expert_current_V = vf.apply(params, Expert_Batch.observations).reshape(-1)
            cost = current_Q - current_V

            w = omega_star(
                current_expert_reward_Q - expert_current_V - current_expert_reward_V
            )
            vf_l2_loss=5000*sum(l2_loss(w) for w in jax.tree_leaves(params["params"]))
            reg_loss = (
                args.l1_ratio * jnp.abs(current_V).mean()
                + jnp.abs(cost).mean()
                + jnp.exp(cost.clip(args.cost_min) - cost.clip(max=args.cost_max)).mean()
            )#+vf_l2_loss
            loss = (
                decay_weight*
                vf_state.opt_state[0].count
        ...
        def actor_loss(params):
            ...
            y = current_reward_Q - current_reward_V +  vf_pi
            w = omega_star(y)
            actor_loss = -(
                jnp.exp(w * 10.0).clip(max=100.0).flatten() * log_prob
            ).mean()
            return actor_loss, -log_prob.mean()
    ...
    actor_state,vf_state,key = update_value_critic_and_actor(
        actor_state,
        qf_state,
        vf_state,
        Policy_Batch,
...
if __name__ == "__main__":
    ...
    expert_obs,expert_next_obs,expert_action,expert_rewards,expert_dones = split_data(expert_data)
