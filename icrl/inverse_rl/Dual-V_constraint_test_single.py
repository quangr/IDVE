...
def plot_vf_state(vf_state, obs):
    ...
    bins = np.linspace(min(xs), max(xs), 10)
...
def train_step(agent_state):
    ...
    def update_critic(
        ...
        #     reward_vf_state.params, Policy_Batch.observations
        # ).reshape(-1)

        policy_next_values = vf.apply(
            vf_state.params, Policy_Batch.next_observations * single_mask
        ).reshape(-1)
        ...
        def mse_loss(params):
            ...
            )#.clip(2*args.cost_min, 2*args.cost_max)
    ...
    def update_value_critic(
        ...
        random_action = jax.random.uniform(key, shape=Expert_Batch.actions.shape,minval=-1,maxval=1)
        random_current_expert_reward_Q = qf.apply(
            reward_qf_state.target_params,
            Expert_Batch.observations,
            random_action,
        ).reshape(-1)
        random_exper_Q = qf.apply(
            qf_state.target_params, Expert_Batch.observations, random_action
        ).reshape(-1)

        def mse_loss(params):
            current_V = vf.apply(params, Batch.observations * single_mask).reshape(-1)
            next_V = vf.apply(params, Batch.next_observations * single_mask).reshape(-1)
            expert_next_V = vf.apply(params, Expert_Batch.next_observations * single_mask).reshape(-1)
            ...
            ) - (((random_action-Expert_Batch.actions)**2)).sum(-1)*omega_star(
                random_current_expert_reward_Q
                + (random_exper_Q - expert_current_V).clip(
                    args.cost_min, args.cost_max
                )
                - current_expert_reward_V
            )
            ...
            loss = decay_weight * args.reg_cof * reg_loss - w.mean()

            return loss, (current_V.mean(), current_Q.mean())

        (vf_loss_value, (vf_values, qf_values)), grads = jax.value_and_grad(
            mse_loss, has_aux=True
        )(vf_state.params)
...
if __name__ == "__main__":
    ...
    random_indice = np.random.randint(0, len(_expert_data["traj_obs"]), size=100)
    ...
    random_indice = np.random.randint(0, len(_expert_data["traj_obs"]), size=1000)
