...
if __name__ == "__main__":
    ...
    def update_critic(
        ...
        def mse_loss(params):
            ...
            discounted_rewards=alpha*Policy_Batch.rewards+(-1/reg_mult)+((1 - Policy_Batch.dones).reshape(-1, 1)*args.gamma*policy_next_values).squeeze(-1)
            loss = ((expert_current_Q - Q_max)**2).mean()+((policy_current_Q.squeeze(-1) - discounted_rewards)**2).mean()
    ...
    def update_value_critic(
        ...
        def mse_loss(params):
            ...
            loss=((1-lamb)*(current_V)+lamb*(((current_Q-current_V)+0.5*(current_Q-current_V)**2).clip(0))).mean()
