...
def parse_args():
    ...
    parser.add_argument("--reward-limit", type=float, default=-4.8,
        help="entropy regularization coefficient")
...
@jax.jit
@checkify.checkify
def train_step(cost_buffer_state,buffer_state, qf_state, reward_qf_state,dual,policy_lag, actor_state,old_actor_state, key):
    sample_key, key = jax.random.split(key, 2)
    observations,next_observations,actions,rewards,costs,dones=cost_buffer.sample_fn(cost_buffer_state, sample_key, args.batch_size)
    sample_key, key = jax.random.split(key, 2)
    ...
    expert_indice =jax.random.randint(sample_key, minval=0, maxval=len(expert_obs), shape=(args.batch_size,))
    ...
    old_mean, old_log_std, actions, update_q,dual = E_step(old_actor_state, qf_state,dual, reward_qf_state, observations, key)
    (actor_state,policy_lag, actor_loss_value, key, entropy,kl) = update_actor(
        actor_state,
        old_mean, old_log_std, actions, update_q,
        policy_lag,
        observations,
        key,
    )
    ...
    return actor_state,qf_state,reward_qf_state,dual,qf_loss_value,qf_values,reward_qf_loss_value, reward_qf_values,actor_loss_value,key,policy_lag,kl
...
if __name__ == "__main__":
    ...
    max_episode_steps=envs.env.config['max_episode_steps']
    reward_limit=args.reward_limit*(1 - args.gamma**max_episode_steps)/(max_episode_steps*(1 -args.gamma)) 
    ...
    cost_buffer_state = cost_buffer.init_fn((obs[0],obs[0],action[0],jnp.array([0.],dtype=jnp.float32),jnp.array([0.],dtype=jnp.float32),jnp.array([True])))
    ...
    def update_actor(
        ...
        def actor_loss(params):
            ...
            log_prob = action_log_prob(actions,old_mean, log_std)+action_log_prob(actions,mean, old_log_std)
            loss_mle=-(log_prob*update_q).mean()
            kl_mu, kl_std = gaussian_kl(old_mean, old_log_std, mean, log_std)
            actor_loss = loss_mle+dual_mu*(kl_mu-0.005).clip(0)+dual_std*(kl_std-0.0005).clip(0)
    ...
    def rollout_cost(obs, episode_stats,buffer_state,cost_buffer_state, actions,handle):
        (
                episode_stats,
                handle,
                (next_obs, rewards, costs, dones, truncated, infos),
        ...
        buffer_state=buffer.add_fn(buffer_state, (obs[0].astype(jnp.float32), next_obs[0].astype(jnp.float32),actions[0].astype(jnp.float32),rewards.astype(jnp.float32),costs.astype(jnp.float32),dones*(1-truncated).astype(bool)))
        cost_buffer_state=cost_buffer.add_fn(cost_buffer_state, (obs[0].astype(jnp.float32), next_obs[0].astype(jnp.float32),actions[0].astype(jnp.float32),rewards.astype(jnp.float32),costs.astype(jnp.float32),dones*(1-truncated).astype(bool)))
        obs=next_obs
        return episode_stats,buffer_state,cost_buffer_state,obs,handle
    ...
    for global_step in generator:
        ...
        episode_stats,buffer_state,cost_buffer_state,obs,handle = rollout_cost(obs,episode_stats, buffer_state,cost_buffer_state, actions,handle)
    ...
    for i in range(20000):
        _,(actor_state,reward_qf_state,reward_qf_loss_value, reward_qf_values,key)=half_train_step(buffer_state, reward_qf_state, actor_state,key)

        # ALGO LOGIC: training.
    @jax.jit
    def train_step_body(carry,step):
        cost_buffer_state,buffer_state, qf_state, reward_qf_state, dual, policy_lag, actor_state, old_actor_state, key = carry
        _, (actor_state, qf_state, reward_qf_state, dual, qf_loss_value, qf_values, reward_qf_loss_value, reward_qf_values, actor_loss_value, key, policy_lag, kl) = train_step(cost_buffer_state,buffer_state, qf_state, reward_qf_state, dual, policy_lag, actor_state, old_actor_state, key)
        return (cost_buffer_state,buffer_state, qf_state, reward_qf_state, dual, policy_lag, actor_state, old_actor_state, key), (qf_loss_value.mean(), qf_values, reward_qf_loss_value, reward_qf_values.mean(), actor_loss_value, kl,dual.state.params)

    # @partial(jax.jit,donate_argnums=(0,1,2,3,4,5,6,7,8),backend="gpu")
    def speeduptrain(cost_buffer_state,buffer_state, qf_state, reward_qf_state, dual, policy_lag, actor_state, old_actor_state, key):
        (cost_buffer_state,buffer_state, qf_state, reward_qf_state, dual, policy_lag, actor_state, old_actor_state, key), (qf_loss_value, qf_values, reward_qf_loss_value, reward_qf_values, actor_loss_value, kl,dual_data) = jax.lax.scan(
            train_step_body,
            (cost_buffer_state,buffer_state, qf_state, reward_qf_state, dual, policy_lag, actor_state, old_actor_state, key),
            (),
            length=50,
        )
        return (cost_buffer_state,buffer_state, qf_state, reward_qf_state, dual, policy_lag, actor_state, old_actor_state, key), (qf_loss_value, qf_values, reward_qf_loss_value, reward_qf_values, actor_loss_value, kl,dual_data)
