...
def parse_args():
    ...
    parser.add_argument("--expert-data", type=str, default="expert_data/BiasedReacher",
...
def plot_vf_state_whole(vf_state, obs):
    ...
    fig, axes = plt.subplots(nrows=plots_num//3+1, ncols=3, figsize=(15,5))
    y = vf.apply(vf_state.params, obs * single_mask)
...
# @checkify.checkify
# def max_reward_train_step(agent_state):
#     callback_log = {}
...
#                 current_reward_Q
#                 - current_reward_V
#                 + (min_qf_pi - vf_pi).clip(args.cost_min, args.cost_max)*10
#             )
#             w = omega_star(y)
#             actor_loss = -(
...
def train_step(agent_state,anti_buffer_state):
    ...
    def update_critic(
        ...
        def mse_loss(params):
            ...
            coeff=0.95
            loss =  coeff*(- w.mean())+vf_l2_loss++(1-coeff)*(((current_Q-next_V)**2).mean())+0.01*((anti_Q-anti_Q_target)**2).mean()+((current_expert_Q-current_expert_Q_target)**2).mean()
...
def rollout_batch(
    ...
    for global_step in generator:
        if random and np.random.rand()<0.2:
...
if __name__ == "__main__":
    ...
    length_deque = collections.deque(maxlen=50)
    ...
    # agentstate=checkpointer.restore("bufferdata/0904", item=agentstate)
    ...
    for global_step in range(int((args.total_timesteps - args.learning_starts) / 50)):
        ...
        log_data = {
            ...
            # "charts/action_diff": np.abs(np.mean(anti_agent_actions[action_size-1]-expert_action)),
    ...
    #         if global_step % 50 == 0:
    #             plot_vf_state(agentstate.vf_state)
    #             wandb.log({"v_plot": wandb.Image(plt)})
    #             plt.close()
