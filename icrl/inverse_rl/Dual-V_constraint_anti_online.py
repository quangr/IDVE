...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="online-ICRL",
    ...
    parser.add_argument("--env-id", type=str, default="Ant-Forward",
        help="the id of the environment to be used")
    parser.add_argument("--expert-state-path", type=str, default="sacpolicy/Ant_expert",
        help="the path of the expert state data")
    parser.add_argument("--expert-data", type=str, default="expert_data/Ant-Forward_expert",
...
#         actor_loss = -(
#             jnp.exp((min_qf_pi - vf_pi) * 10.0).clip(max=100.0).flatten() * log_prob
#         ).mean()
...
def plot_vf_state_whole(vf_state,qf_state, obs,action):
    plots_num=obs.shape[-1]
    fig, axes = plt.subplots(nrows=3, ncols=(plots_num+2)//3, figsize=(30,10))
    y = qf.apply(qf_state.target_params, obs ,action).squeeze(axis=-1).min(0)-vf.apply(vf_state.params, obs * single_mask).reshape(-1)
...
def reward_train_step(agent_state):
    ...
    def update_reward_critic(
        ...
        # next_cost_values = vf.apply(vf_state.params, Batch.next_observations*single_mask).reshape(
        #     -1
        # )
    ...
    def update_actor(
        ...
        def actor_loss(params):
            ...
            max_qf_pi = qf.apply(qf_state.target_params, Batch.observations, Batch.actions).squeeze(axis=-1).min(0)
            y = (
                current_reward_Q
                - current_reward_V
                + (max_qf_pi - vf_pi).clip(args.cost_min, args.cost_max)
            )
            w = omega_star(y)
            actor_loss = -(
                w.flatten() 
                # jnp.exp(y*10).clip(10).flatten()
                * log_prob
            ).mean()
            return actor_loss, -log_prob.mean()
...
def train_step(agent_state,anti_buffer_state):
    ...
    def update_value_critic(
        ...
        decay_weight = (
            0.8 * decay_weight + vf_state.opt_state[0].count / args.total_timesteps
        ).clip(max=1)
        ...
        def mse_loss(params):
            ...
            return loss+cost, jnp.concatenate([expert_current_Q, policy_current_Q]).mean()
    ...
    def update_critic(
        ...
        def mse_loss(params):
            ...
            coeff=0.95
            loss =  coeff*(- w.mean())+vf_l2_loss+(1-coeff)*(jnp.abs(anti_Q).mean()+jnp.abs(current_expert_Q).mean())
...
def rollout_batch(
    ...
    return key, obs, cum_reward, cum_cost, lengths, agentstate
...
if __name__ == "__main__":
    ...
    optimal_actor_state = state["actor_state"]
    ...
    generator = tqdm(range(30000))
    key, obs, cum_reward, cum_cost, lengths, agentstate = rollout_batch(
        key,
        envs,
        obs,
        reward_deque,
    ...
    agentstate = agentstate._replace(
        ...
        # actor_state=checkpointer.restore("/home/guorui/jax-rl/bcpolicy/BiasedReacher/BiasedReacher__bahavior_clone__2__16930289954/", item=actor_state)
    )
    generator = tqdm(range(int(50000)))
    key, obs, cum_reward, cum_cost, lengths, agentstate = rollout_batch(
        key,
        envs,
        obs,
    ...
    # save_args = orbax_utils.save_args_from_target(agentstate)
    # checkpointer.save("bufferdata/0904", agentstate, save_args=save_args)
    ...
    agentstate = agentstate._replace(buffer_state=checkpointer.restore("bufferdata/buffer_state", item=agentstate.buffer_state))
    ...
    # def max_reward_train_step_body(carry, step):
    #     agentstate = carry
    #     _, (agentstate, callback_log) = _max_reward_train_step(agentstate)
    #     return agentstate, callback_log
    ...
    # anti_agent_actions=jnp.empty((1000,*expert_action.shape))
    # bad_index=(jnp.abs((anti_agent_actions[0]-expert_action)).sum(-1)>0.2)
    @partial(jax.jit, donate_argnums=(0))
    ...
    # anti_agent_actions = anti_agent_actions.at[0].set(sample_action(actor, agentstate.actor_state, expert_obs, key)[0])
    anti_buffer_state = update_anti_actions(anti_buffer_state,agentstate.actor_state)
    # action_size=1
    reward_deque = collections.deque(maxlen=500)
    cost_deque = collections.deque(maxlen=500)
    length_deque = collections.deque(maxlen=500)
    ...
    for global_step in range(int((args.total_timesteps - args.learning_starts) / 50)):
        ...
        if args.track:
            ...
            # if global_step % 50 == 0:
            #     plot_vf_state(agentstate.vf_state, total_obs)
    ...
    if args.track:
        # Assuming you have imported the required libraries (e.g., wandb)
        plot_vf_state_whole(agentstate.vf_state,agentstate.qf_state, total_obs,total_action)
        wandb.log({"all_plot": wandb.Image(plt)})
        plt.close()
    state = {
